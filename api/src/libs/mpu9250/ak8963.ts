...
export class ak8963 {
	...
	public initialize(): boolean {
		if (this.isEnabled && !this.isReady) {
			...
			if (buffer === AK8963Map.WHO_AM_I_RESPONSE) {
				...
				this.debug.log(
					"error",
					`AK8963: Device ID is not equal to 0x${AK8963Map.WHO_AM_I_RESPONSE.toString(16)}, device value is 0x${buffer.toString(16)}`
				)
				return false
			}
		} else {
	...
	public setCNTLAsync(mode: number): Promise<number | false> {
		if (this.i2c) {
			return this.i2c.writeBytesAsync(AK8963Map.CNTL, [mode]).then(() => {
				return mode
			})
		}
		return Promise.resolve(false)
